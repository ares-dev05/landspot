...
class ShareClientAccessModal extends React.Component {
    static propTypes = {
        onCancel: PropTypes.func.isRequired,
        getAvailableManagers: PropTypes.func.isRequired,
        shareBuyerCredentials: PropTypes.func.isRequired,
    ...
    componentDidMount() {
        const {userActionData, getAvailableManagers} = this.props;

        getAvailableManagers(userActionData);
    }
    ...
    componentDidUpdate(prevProps) {
        ...
        if (prevProps.loading && !loading) {
            const selectedManagers = managers.filter(
                user => user.available_invited_users.length !== 0
            );
    ...
    shareAccess = () => {
        const {userActionData, shareBuyerCredentials} = this.props;
        const {selectedManagers, deletedManagers} = this.state;

        shareBuyerCredentials({selectedManagers, deletedManagers}, userActionData);
    };
                    ...
                    {managers
                        ? managers.map(
                            manager => {
                                ...
                                const shared = manager.available_invited_users.length !== 0;
