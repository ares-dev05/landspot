...
class PagePreview extends Component {
    ...
    addNearmapModelListener() {
        ...
        if (nearmapModel) {
            nearmapModel.addEventListener(EventBase.CHANGE, this.onNearmapModelChanged, this);
        }
    ...
    removeNearmapModelListener() {
        const {nearmapModel} = CanvasModel.getModel();
        if (nearmapModel) {
            nearmapModel.removeEventListener(NearmapModel.LOCATION_CHANGE, this.onNearmapModelChanged, this);
        }
        if (this.nearmapView) {
    ...
    componentDidUpdate(prevProps) {
        ...
        if (prevProps.nearmapsVisualisationEnabled !== nearmapsVisualisationEnabled && !nearmapsVisualisationEnabled) {
        ...
        switch (currentTab) {
            ...
            case tabs.ENGINEERING.id: {
                ...
                if (prevProps.engineeringEnabled !== engineeringEnabled && engineeringEnabled) {
                    state = this.enableEngineeringView();

                    if (this.alignEngineeringView) {
                        this.alignEngineeringView.pdfPage = page;

                        if (state && !heightVisualisationEnabled && !threeDVisualisationEnabled && !nearmapsVisualisationEnabled) {
            ...
            case tabs.HEIGHT_VISUALISATION.id: {
                if (prevProps.heightVisualisationEnabled !== heightVisualisationEnabled && heightVisualisationEnabled) {
                    this.enableEnvelopeCanvasView();
                }
                break;
            }
            case tabs.THREE_D_VISUALISATION.id: {
                if (prevProps.threeDVisualisationEnabled !== threeDVisualisationEnabled && threeDVisualisationEnabled) {
    ...
    enableTraceView(traceEnabled) {
        ...
        if (pixiApp) {
            if (!this.canvasTraceView) {
                this.canvasTraceView = new SitingsTraceView(CanvasModel.getModel(), this.pixiApp);
            }
    ...
    saveNearmapOverlay() {
        ...
        const W = this.nearmapView.sizedWidth, H = this.nearmapView.sizedHeight;
        ...
        ctx.drawImage(lotBackground, 0, 0, lotBackground.width, lotBackground.height);
        ctx.restore();
    ...
    enableEngineeringView() {
        ...
        if (pixiApp) {
            ...
            if (!this.alignEngineeringView) {
                this.alignEngineeringView = new AlignEngineeringView(canvasModel, this.pixiApp);
            }
    ...
    setZoom = (v) => {
        let {resizeTS} = this.state;
        let {drawerData: {zoom}, setDrawerData} = this.props;
        zoom = v === 0 ? 100 : zoom + v;
    ...
    render() {
        ...
        let {resizeTS, resetPage, rotation, scale, planScale, moveSetting, nearmapLocation, nearmapAutoPlacement} = this.state;

        const page = getPage();

        // indicates if any of the advanced modes are currently opened
        const inAdvancedMode = engineeringEnabled || heightVisualisationEnabled || threeDVisualisationEnabled || nearmapsVisualisationEnabled;

        // indicates if trace view is currently disabled
        const traceViewDisabled   = !traceEnabled && !inAdvancedMode;

        const hasAdvancedFeatures = AccountMgr.i.builder && AccountMgr.i.builder.hasAdvancedFeatures;
        const hasHeightEnvelope   = AccountMgr.i.builder && AccountMgr.i.builder.hasHeightEnvelope;
        const hasNearmapOverlay   = AccountMgr.i.builder && AccountMgr.i.builder.hasNearmapOverlay;
        ...
        return (
            <ResizableBox className="page-preview"
                ...
                <React.Fragment>
                    ...
                    <div>
                        <div
                            className={classnames('trace-view', traceViewDisabled && 'disabled')}
                        ...
                        {
                            (engineeringEnabled || hasNearmapControls) && !heightVisualisationEnabled && !threeDVisualisationEnabled &&
                            <div className="sitings-sliders"
                                ...
                                <div className="btn-group" style={{display: 'block'}}>
                                    <ToggleSwitch
                                        ...
                                        text={{on: 'Move Siting', off: engineeringEnabled ? 'Move Workspace' : 'Move Map'}}
                                        label={{on: '', off: ''}}
                                        state={moveSetting}
                                        style={{marginTop: '10px'}}
                                    />

                                    <span>&nbsp;&nbsp;&nbsp;(You can also use the Arrow Keys to move)</span>
...
const Slider = ({value, label, onSlideEnd, onUpdate, formatter, min = -180, max = 180, step = 0.01}) => {
    return (
        <CompoundSlider min={min} max={max} step={step}
