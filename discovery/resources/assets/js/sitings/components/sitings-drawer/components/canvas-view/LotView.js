...
class LotView extends Component {
    ...
    componentDidUpdate(prevProps) {
        const {
            drawerDetails, step, drawerData
        } = this.props;

        if (drawerDetails.EXPORT_LOT_DATA && !prevProps.drawerDetails.EXPORT_LOT_DATA) {
    ...
    componentWillUnmount() {
        ...
        if (this.multiFloors) {
            this.multiFloors.removeEventListener('floorRotated', this.onFloorRotated, this);
            this.multiFloors.removeEventListener(ModelEvent.SELECT, this.newHouseModelSelected, this);
            this.multiFloors = null;
        }
        if (this.measurementsModel) {
            this.measurementsModel.removeEventListener(EventBase.CHANGE, this.onMeasurementModelChanged, this);
            this.measurementsModel.removeEventListener(EventBase.ADDED, this.onMeasurementModelChanged, this);
            this.measurementsModel = null;
        }
    ...
    exportLotData = (exportType) => {
        ...
        switch (exportType) {
            ...
            case UserAction.SAVE_AND_EXPORT: {
                ...
                setDrawerData(
                    { sitingImage, engineeringImage, nearmapImage },
    ...
    _renderViewToSvg = (view, printDpi = 144) => {
        ...
        const svgData = (new XMLSerializer()).serializeToString(this.canvasSvgView.svgNode);
        div.remove();
    ...
    drawEngineeringImage = () => {
        ...
        if (model.engineeringState && model.engineeringState.view) {
            ...
            const render = this._renderViewToSvg(model.engineeringState.view, 96 /*web print DPI, needed for correct PhantomJS export, due to zoom set by rasterize.js*/);
            ...
            */
            ...
            const template =
                ...
                '        .image {\n' +
                '            overflow: hidden;' +
                '            width: ' + (config.pageSize.width - Math.round(config.underlay.e)) + 'px;\n' +
                '            height: ' + (config.pageSize.height - Math.round(config.underlay.f)) + 'px;\n' +
                '        }\n' +
                '       img {' +
                '           width: ' + engineeringWidth + 'px;' +
                '           height: ' + engineeringHeight + 'px;' +
                '           position: absolute;' +
                '           left: ' + Math.round(config.underlay.e) + 'px;' +
                '           top: ' + Math.round(config.underlay.f) + 'px;' +
                '       }' +
                '       svg {' +
    ...
    setupCanvasView = () => {
        ...
        this.northIndicator.addListener(EventBase.CHANGE, this.northRotationChanged, this);
        this.setupModelStep();

        canvasModel.multiFloors.addEventListener('floorRotated', this.onFloorRotated, this);
        canvasModel.multiFloors.addEventListener(ModelEvent.SELECT, this.newHouseModelSelected, this);

        canvasModel.measurementsModel.addEventListener(EventBase.CHANGE, this.onMeasurementModelChanged, this);
        canvasModel.measurementsModel.addEventListener(EventBase.ADDED, this.onMeasurementModelChanged, this);
    ...
    updateStage = () => {
        ...
        if (lotBackground) {
            if (this.props.step === ApplicationStep.EXPORT_PDF) {
                lotBackground.texture =
                    siting.templateImage
        ...
        if (this.canvasView.viewScale !== viewScale &&
            !engineeringEnabled &&
            !heightVisualisationEnabled &&
            !threeDVisualisationEnabled &&
            !nearmapsVisualisationEnabled) {
            console.log('resetting viewScale and centering ', this.canvasView.viewScale, viewScale);
    ...
    render() {
        ...
        const disablePageAlignButtons = !pageAlignMode || canvasModel.measurementsModel.currentPoint === null;

        let houseRotation = 0;
        try { houseRotation = canvasModel.multiFloors.crtFloor.rotation; } catch (e) { }
        ...
        return (
            <React.Fragment>
                <div className={classnames('lot-view', isExport && 'export')}
                    ref={node => this.pixiElement = node} />

                {restored &&
                    <SiteCoverage full={isExport} />
                }

                {isExport &&
                    <div id='export-preview' className={classnames('export-preview', !sitingPage && 'hidden')} />
                }

                {isExport &&
                    <div id='engineering-preview' className={classnames('export-preview', 'solid', sitingPage && 'hidden')} />
                }
                ...
                {pageAlignMode &&
                    <PageAlignButtons alignPage={alignPage}
                        disabled={disablePageAlignButtons} />
                }

                <LotControls houseRotation={houseRotation}
...
const PageAlignButtons = ({ alignPage, disabled }) => {
    return <div className={classnames('align-to-page', disabled && 'disabled')}>
        <button type="button" className='button transparent left' title='Align to Left'
            onClick={() => alignPage(MeasurementsLayerModel.DIRECTION_LEFT)}>
            <i className="landconnect-icon arrow-left" />
        </button>

        <button type="button" className='button transparent right' title='Align to Right'
            onClick={() => alignPage(MeasurementsLayerModel.DIRECTION_RIGHT)}>
            <i className="landconnect-icon arrow-right" />
        </button>

        <button type="button" className='button transparent top' title='Align to Top'
            onClick={() => alignPage(MeasurementsLayerModel.DIRECTION_UP)}>
            <i className="landconnect-icon arrow-left" />
        </button>

        <button type="button" className='button transparent bottom' title='Align to Bottom'
...
const SiteCoverage = ({ full }) => {
    ...
    const areas = [
        ...
        { label: 'Ext/Red', area: siteCalculator.transformArea + siteCalculator.structureArea },
    ...
    return (
        <div className="site-coverage">
            <table>
                <tbody>
                    <tr>
                        <th><b>SITE COVERAGE</b></th>
                        <th><b>{coverage.toFixed(2)}%</b></th>
                    </tr>
...
const LotControls = ({ checkRotation, step, canvasView = {}, houseRotation = 0, northRotation = 0 }) => {
    const pagePreview = document.querySelector('.page-preview');
    let leftMargin = 30;
                    ...
                    return (
                        <div className='lot-controls'>
                            ...
                            <ZoomButtons setZoom={scaleModel}
                                previewHeight={previewHeight + 60} disabled={step === ApplicationStep.EXPORT_PDF} />
...
const Slider = ({ value, label, onSlideEnd, onUpdate }) => {
    return (
        <CompoundSlider min={-180} max={180} step={0.01}
...
const NorthIndicator = ({ angle }) => (
    <svg width="40px"
        height="40px"
        viewBox="-20 -20 40 40" version="1.1" xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
        style={{
            transform: `rotate(${angle * Math.PI / 180}rad)`
        }}>
        <g id="xxhdpi/ic_launcher_APP" stroke="none" strokeWidth="0"
            fill="none" fillRule="evenodd">
            <circle id="circle" cx="0" cy="0" r="20" />
            <polygon id="arrow" fill="white" fillRule="nonzero"
...
const SiteCoverageWarning = () => (
    <svg height="11"
        width="12.5" viewBox="0 0 627.769 550.45" version="1.0" xmlns="http://www.w3.org/2000/svg">
        ...
        <polygon id="polygon2233" points="93.977 482.88 533.9 482.88 313.94 101.89" fill="#ea0000" />
        <path d="m291.87 343.36c1.21 11.49 3.21 20.04 6.02 25.66 2.81 5.63 7.82 8.43 15.04 8.43h2.01c7.22 0 12.24-2.8 15.04-8.43 2.81-5.62 4.82-14.17 6.02-25.66l6.42-88.75c1.21-17.3 1.81-29.71 1.81-37.25 0-10.25-2.91-18.25-8.73-23.99-5.53-5.46-13.38-8.59-21.56-8.59s-16.04 3.13-21.57 8.59c-5.81 5.74-8.72 13.74-8.72 23.99 0 7.54 0.6 19.95 1.8 37.25l6.42 88.75z" id="path2235" fill="#fff" />
