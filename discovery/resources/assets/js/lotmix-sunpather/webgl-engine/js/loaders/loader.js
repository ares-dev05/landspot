...
export default class Loader {
  ...
  loadModel(prefix='', dracoPath='/draco/') {
    ...
    loader.load(
      ...
      gltfModel => {
        ...
        //let box = model.getSize(measure);
        //console.log( box, measure );
        ...
        if (this.api.lot === 'small') {
          ...
          const SCK = 1.28;
          model.scale.set(this.api.scale * SCK, this.api.scale * SCK, this.api.scale * SCK);
          model.position.set(
              (this.api.zPosition - 3.05 ) * this.api.scale,
              this.api.yPosition + 0.1,
              (this.api.xPosition + 14.55) * this.api.scale,
          );
        }
        ...
        if (this.viewer) {
          document.addEventListener('click', event => {
            ...
            mouse.x = (event.layerX / this.viewer.renderer.domElement.offsetWidth) * 2 - 1;
            mouse.y = -(event.layerY / this.viewer.renderer.domElement.offsetHeight) * 2 + 1;
            ...
            if (intersectsScene[0]) {
              ...
              window.selectedMesh = intersectsScene[0].object;
              conf.color =
                  '#' + window.selectedMesh.material.color.getHexString();
              conf.metalness = window.selectedMesh.material.metalness;
              conf.roughness = window.selectedMesh.material.roughness;
              conf.emissive =
                  '#' + window.selectedMesh.material.emissive.getHexString();
        ...
        console.log('house model is ready ', group, model);
        console.log(this.api, ' test ', ('onLoaded' in this.api));
        ...
        } else if ('onLoaded' in this.api) {
          this.api.onLoaded(group);
        }
      },
      xhr => {
